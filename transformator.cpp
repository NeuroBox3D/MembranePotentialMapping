...
/// TODO: Use the ForAllSections(sec) construct to replace the cumbersome for loop constructs in the other methods if necessary!
/// NOTE: Section* sec = chk_access() and secname(sec) will give the currently 
///       accessed session, this could be handy in future, e. g. for the clamp constructs
...
#include "section.h"
// #include "cabcode.c" -> depends on python probably and we do not need it
...
#endif
//void ivoc_cleanup() { return; } // TODO: UNDEF this afterwards (When fixing includes in ivoc.cpp it becomes available!)
                  ...
                  for (size_t j=0; j <= no_points; j++) {
                    ...
                    for(TetrahedronIterator iter = sh.begin<Tetrahedron>(0); iter != sh.end<Tetrahedron>(0); ++iter) {
                      ...
                      if (ContainsPoint(h, neuron_point, aaPos)) {
                        ...
                        // double Ca_i = 0.;
                        // TODO: integrate on subset with "subset name" with integrate.h from Andreas Vogel
                        ...
                        // TODO: For this to work specify channel model first with NMDL language of NEURON 
                        // stmt << "forsec current_section { Cai = " << Ca_i << "}"; 
                        break;
                      }
                    }
                  } // for points
...
void Transformator::prepare() {
   ...
   m_stmts.push_back("current_section_index = 0"); // Note: Was this correct here previously we excluded this?
