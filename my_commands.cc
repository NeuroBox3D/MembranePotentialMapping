...
double BG::getNeumannFlux(double time, double delta_t, double Vm) {
...
double BG::getCurrent(double time, double delta_t, double Vm) {
   ...
   for (unsigned int i=0; i < solgat.size(); i++)
      flux+=solgat[i].calculate_current_expli_next_timestep(time_in_ms, delta_t_in_ms, Vm);
 ...
 //  Neumann_flux = solgat[0].calculate_current_expli_next_timestep( time_in_ms, delta_t_in_ms, Vm ); 
   ...
   for (unsigned int i=0; i < solgat.size(); i++)
      current += solgat[i].Current_current(time_in_ms);
