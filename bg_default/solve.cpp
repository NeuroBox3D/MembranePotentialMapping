...
{
    return - g * pow( x, xp ) * pow( y, yp ) * ( BG::Voltage(myVm) - V_ret );
}
...
double solve_gating::initial_value_for_current(double basic_voltage, double myVm, double Ca_i, double Ca_o) {
	 current_x_0 = gate_x.x_infty( basic_voltage );
	  current_y_0 = gate_y.x_infty( basic_voltage );
...
double solve_gating::calculate_current_expli_next_timestep( double time, double delta_t, double myVm, double Ca_i, double Ca_o )
{
