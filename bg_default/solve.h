...
public:
		...
		solve_gating( gating_parameter gate_x_, gating_parameter gate_y_, double g_, double V_ret_, int xp_, int yp_ )
			: gate_x( gate_x_ ), gate_y( gate_y_ ), g(g_), V_ret(V_ret_), xp(xp_), yp(yp_), d_Ca(0.0)
		{};

		solve_gating( gating_parameter gate_x_, gating_parameter gate_y_, double g_, double p_, double val, double V_ret_, int xp_, int yp_ )
			: gate_x( gate_x_ ), gate_y( gate_y_ ), g(g_), permeability(p_), valency(val), V_ret(V_ret_), xp(xp_), yp(yp_), d_CadCa_i(0.0), d_CadCa_o(0.0)
		{};


        double initial_value_for_current( double basic_voltage = -65, double myVm = -75 ); // t = 0, uses -65 mV as standard value may be changed
        double initial_value_for_current(double basic_voltage, double myVm, double Ca_i, double Ca_o);
        double calculate_current_expli_next_timestep( double time, double delta_t , double myVm);
        double calculate_current_expli_next_timestep( double time, double delta_t, double myVm, double Ca_i, double Ca_o);

        double current_x() { return current_x_0; }
        double current_y() { return current_y_0; }

        double dCadCa_i() const { return d_CadCa_i; }
        double dCadCa_o() const { return d_CadCa_o; }
        double dCa() const { return d_Ca; }

        double Current_current( double time, double myVm) { return I(  current_x_0, current_y_0, time ,  myVm); }
        double Current_current(double time, double myVm, double Ca_i, double Ca_o) { return I(current_x_0, current_y_0, time, myVm, Ca_i, Ca_o); }

private:

	double I( double x, double y, double t, double myVm );
	double I(double x, double y, double t, double myVm, double Ca_i, double Ca_o);
